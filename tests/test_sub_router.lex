...
fn list_h(_c :: ctx.Ctx) -> resp.Response { resp.text("list") }
fn one_h(_c  :: ctx.Ctx) -> resp.Response { resp.text("one") }
fn create_h(_c :: ctx.Ctx) -> resp.Response { resp.text("create") }
...
fn test_prefix_join_index() -> Result[Unit, Str] {
  ...
  if r.status == 200 and r.body == "list" { Ok(()) }
  else { Err("index route did not match under prefix") }
}

fn test_prefix_join_param() -> Result[Unit, Str] {
  let r := router.dispatch_pure(app(), t.get("/users/42"))
  if r.status == 200 and r.body == "one" { Ok(()) }
  else { Err(":id route did not match under prefix") }
}

fn test_post_under_prefix() -> Result[Unit, Str] {
  let r := router.dispatch_pure(app(), t.post("/users", "{}"))
  if r.status == 200 and r.body == "create" { Ok(()) }
  else { Err("POST under prefix did not match") }
}
...
fn test_meta_summary_attached() -> Result[Unit, Str] {
  ...
  let found := list.fold(routes, false,
    fn (acc :: Bool, rec :: router.RouteRecord) -> Bool {
      acc or (rec.method == "GET" and rec.pattern == "/users"
              and rec.meta.summary == "List users")
    })
  if found { Ok(()) } else { Err("summary metadata missing") }
}
...
fn test_meta_status_attached() -> Result[Unit, Str] {
  ...
  let found := list.fold(routes, false,
    fn (acc :: Bool, rec :: router.RouteRecord) -> Bool {
      acc or (rec.method == "POST" and rec.pattern == "/users"
              and rec.meta.status == 201)
    })
  if found { Ok(()) } else { Err("status metadata missing") }
}
...
fn suite() -> List[Result[Unit, Str]] {
  [
    test_prefix_join_index(),
    test_prefix_join_param(),
    test_post_under_prefix(),
    test_meta_summary_attached(),
    test_meta_status_attached(),
    test_meta_tags_attached(),
  ]
}
