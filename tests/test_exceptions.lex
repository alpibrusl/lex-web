...
fn registry() -> ex.Registry[AppError] {
  ex.new()
    |> fn (r :: ex.Registry[AppError]) -> ex.Registry[AppError] {
...
fn test_first_matcher_wins() -> Result[Unit, Str] {
  ...
  if r.status == 404 and str.contains(r.body, "user") { Ok(()) }
  else { Err("not_found matcher missed") }
}

fn test_second_matcher() -> Result[Unit, Str] {
  let r := ex.handle(registry(), Conflict("name taken"))
  if r.status == 409 and str.contains(r.body, "name taken") { Ok(()) }
  else { Err("conflict matcher missed") }
}

fn test_default_fallback_500() -> Result[Unit, Str] {
  let r := ex.handle(registry(), RateLimited)
  if r.status == 500 { Ok(()) }
  else { Err("expected default 500 fallback") }
}

fn test_custom_fallback() -> Result[Unit, Str] {
  let reg := ex.with_fallback(registry(),
    fn (_e :: AppError) -> resp.Response { resp.json_status(503, "{}") })
  let r := ex.handle(reg, RateLimited)
  if r.status == 503 { Ok(()) }
  else { Err("custom fallback ignored") }
}

fn test_handle_result_ok() -> Result[Unit, Str] {
  let r := ex.handle_result(registry(), Ok(7),
    fn (n :: Int) -> resp.Response { resp.text(int.to_str(n)) })
  if r.status == 200 and r.body == "7" { Ok(()) }
  else { Err("handle_result ok branch wrong") }
}

fn test_handle_result_err() -> Result[Unit, Str] {
  let r := ex.handle_result(registry(),
    Err(NotFound("post")),
    fn (_v :: Int) -> resp.Response { resp.text("unreached") })
  if r.status == 404 { Ok(()) }
  else { Err("handle_result err branch wrong") }
}

fn suite() -> List[Result[Unit, Str]] {
  [
    test_first_matcher_wins(),
    test_second_matcher(),
    test_default_fallback_500(),
    test_custom_fallback(),
    test_handle_result_ok(),
    test_handle_result_err(),
  ]
}
