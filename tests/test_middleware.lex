...
#
# apply_post is [io, time] because MwRequestId uses time.now().
# Tests that call apply_post therefore also declare [io, time].
...
fn repeat_str(s :: Str, n :: Int) -> Str {
  if n <= 0 { "" }
  else { str.concat(s, repeat_str(s, n - 1)) }
}
...
fn cors_adds_origin_header() -> [io, time] Result[Unit, Str] {
  let c  := bare_ctx("/", "")
  let r  := resp.json("{}")
...
fn cors_adds_methods_header() -> [io, time] Result[Unit, Str] {
  let c  := bare_ctx("/", "")
  let r  := resp.json("{}")
...
fn cors_preserves_existing_headers() -> [io, time] Result[Unit, Str] {
  let c  := bare_ctx("/", "")
...
fn request_id_adds_header() -> [io, time] Result[Unit, Str] {
  let c  := bare_ctx("/", "")
  let r  := resp.json("{}")
...
fn body_limit_is_noop_in_post() -> [io, time] Result[Unit, Str] {
  let c  := bare_ctx("/", "")
...
fn suite() -> [io, time] List[Result[Unit, Str]] {
  [
