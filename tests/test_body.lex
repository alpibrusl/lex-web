...
fn json_ctx(body_str :: Str) -> ctx.Ctx {
  ctx.from_request_with_headers({ method: "POST", path: "/", body: body_str, query: "", headers: map.new() }, map.new(), map.from_list([("content-type", "application/json")]))
}

fn form_ctx(body_str :: Str) -> ctx.Ctx {
  ctx.from_request_with_headers({ method: "POST", path: "/", body: body_str, query: "", headers: map.new() }, map.new(), map.from_list([("content-type", "application/x-www-form-urlencoded")]))
}
...
fn is_json_true_for_json_content_type() -> Result[Unit, Str] {
  ...
  if body.is_json(c) {
    ...
    Err("expected is_json to be true")
  }
}

fn is_json_false_for_form_content_type() -> Result[Unit, Str] {
  let c := form_ctx("a=1")
  if not body.is_json(c) {
    Ok(())
  } else {
    Err("expected is_json to be false for form")
  }
}
...
fn is_form_true_for_urlencoded() -> Result[Unit, Str] {
  ...
  if body.is_form(c) {
    ...
    Err("expected is_form to be true")
  }
}
...
fn require_json_body_ok_returns_json() -> Result[Unit, Str] {
  ...
  match body.require_json_body(c, tf.name_validator()) {
    Ok(_) => Ok(()),
    Err(r) => Err(str.concat("expected ok, got status ", int.to_str(r.status))),
  }
}
...
fn raw_body_returns_body_string() -> Result[Unit, Str] {
  ...
  if body.raw_body(c) == "hello world" {
    Ok(())
  } else {
    Err("raw_body did not return original body")
  }
}
...
fn form_body_raw_parses_multiple_pairs() -> Result[Unit, Str] {
  ...
  let m := body.form_body_raw(c)
  let a := match map.get(m, "a") {
    Some(v) => v,
    None => "",
  }
  let b := match map.get(m, "b") {
    Some(v) => v,
    None => "",
  }
  if a == "1" and b == "hello" {
    ...
    Err(str.concat("unexpected values: a=", str.concat(a, str.concat(" b=", b))))
  }
}

# ---- Suite -------------------------------------------------------
fn suite() -> List[Result[Unit, Str]] {
  [is_json_true_for_json_content_type(), is_json_false_for_form_content_type(), is_form_true_for_urlencoded(), json_body_ok_on_valid_json(), json_body_err_on_invalid_json(), json_body_err_on_missing_required_field(), require_json_body_ok_returns_json(), require_json_body_err_returns_422(), raw_body_returns_body_string(), form_body_raw_parses_simple_pair(), form_body_raw_parses_multiple_pairs()]
}
