...
fn test_serves_html() -> Result[Unit, Str] {
  ...
  if r.status == 200 and str.contains(r.body, "home")
     and map.get(r.headers, "content-type") == Some("text/html; charset=utf-8") { Ok(()) }
  else { Err("html not served correctly") }
}

fn test_serves_css() -> Result[Unit, Str] {
  let r := router.dispatch_pure(app(), t.get("/static/css/site.css"))
  if r.status == 200
     and map.get(r.headers, "content-type") == Some("text/css; charset=utf-8") { Ok(()) }
  else { Err("css content-type wrong") }
}

fn test_serves_js() -> Result[Unit, Str] {
  let r := router.dispatch_pure(app(), t.get("/static/script.js"))
  if r.status == 200
     and map.get(r.headers, "content-type")
       == Some("application/javascript; charset=utf-8") { Ok(()) }
  else { Err("js content-type wrong") }
}

fn test_404_unknown() -> Result[Unit, Str] {
  let r := router.dispatch_pure(app(), t.get("/static/missing.txt"))
  if r.status == 404 { Ok(()) } else { Err("expected 404") }
}

fn test_traversal_blocked() -> Result[Unit, Str] {
  let r := router.dispatch_pure(app(),
    t.get("/static/..%2Fsecret"))
  # Even after URL decoding, lex-web sees the literal `..` segment.
  if r.status == 400 or r.status == 404 { Ok(()) }
  else { Err("traversal not blocked") }
}
