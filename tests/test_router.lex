...
fn echo_two_params(c :: ctx.Ctx) -> resp.Response {
  let org := match ctx.path_param(c, "org")  { Some(s) => s, None => "" }
  let uid := match ctx.path_param(c, "user") { Some(s) => s, None => "" }
...
fn param_route_matches_and_binds() -> Result[Unit, Str] {
  ...
  t.all([
    t.assert_status(resp, 200),
    t.assert_body_eq(resp, "42"),
  ])
}

fn param_does_not_match_empty_segment() -> Result[Unit, Str] {
  # `/users/` with trailing slash splits to ["users",""] —
  # the empty segment should not bind as an id; result is 404.
...
fn splat_captures_single_segment() -> Result[Unit, Str] {
  ...
  t.all([
    t.assert_status(resp, 200),
    t.assert_body_eq(resp, "readme.txt"),
  ])
}
...
fn splat_captures_multiple_segments() -> Result[Unit, Str] {
  ...
  t.all([
    t.assert_status(resp, 200),
    t.assert_body_eq(resp, "a/b/c.txt"),
  ])
}

fn two_params_both_bound() -> Result[Unit, Str] {
  let resp := router.dispatch_pure(
    simple_router(), t.get("/orgs/acme/users/alice"))
  t.all([
    t.assert_status(resp, 200),
    t.assert_body_eq(resp, "acme/alice"),
  ])
}

fn registration_order_first_wins() -> Result[Unit, Str] {
  # Register /items/:id first, then /items/special. The first
  # registered match wins in v0.1 flat routing.
  let r :=
    router.new()
      |> fn (r :: router.Router) -> router.Router {
           router.route(r, "GET", "/items/:id",
             fn (c :: ctx.Ctx) -> resp.Response { resp.text("param") })
         }
      |> fn (r :: router.Router) -> router.Router {
           router.route(r, "GET", "/items/special",
             fn (c :: ctx.Ctx) -> resp.Response { resp.text("static") })
         }
  let resp := router.dispatch_pure(r, t.get("/items/special"))
  t.assert_body_eq(resp, "param")
}

fn static_before_param_wins() -> Result[Unit, Str] {
  # Register /items/special before /items/:id — static wins.
  let r :=
    router.new()
      |> fn (r :: router.Router) -> router.Router {
           router.route(r, "GET", "/items/special",
...
fn method_is_case_insensitive() -> Result[Unit, Str] {
  # Route registered as "get" should match a "GET" request.
  let r := router.new()
    |> fn (r :: router.Router) -> router.Router {
         router.route(r, "get", "/ping",
           fn (c :: ctx.Ctx) -> resp.Response { resp.text("pong") })
       }
...
fn suite() -> List[Result[Unit, Str]] {
  [
    static_route_matches(),
    static_route_not_found(),
    param_route_matches_and_binds(),
    param_does_not_match_empty_segment(),
    method_mismatch_gives_404(),
    post_matches_own_method(),
    splat_captures_single_segment(),
    splat_captures_multiple_segments(),
    two_params_both_bound(),
    registration_order_first_wins(),
    static_before_param_wins(),
    method_is_case_insensitive(),
  ]
}
