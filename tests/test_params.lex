...
fn test_query_int_missing_no_default() -> Result[Unit, Str] {
  ...
  match params.query_int(c, "page", None, []) {
    Err(r) => if r.status == 422 { Ok(()) } else { Err("expected 422") },
...
fn test_query_str_default() -> Result[Unit, Str] {
  ...
  match params.query_str(c, "name", Some("anon"), []) {
    Ok(v)  => if v == "anon" { Ok(()) } else { Err("wrong default") },
    Err(_) => Err("expected default"),
  }
}
...
fn test_bearer_present() -> Result[Unit, Str] {
  let req := t.request_with_headers("GET", "/", "", "",
...
fn test_bearer_missing() -> Result[Unit, Str] {
  let c := ctx_for(t.get("/"))
  match params.bearer(c) {
    Err(r) => if r.status == 401 { Ok(()) } else { Err("expected 401") },
...
fn suite() -> List[Result[Unit, Str]] {
  [
    test_query_int_present(),
    test_query_int_default(),
    test_query_int_missing_no_default(),
    test_query_int_invalid(),
    test_query_int_constraint_fail(),
    test_query_str_default(),
    test_query_optional_int_absent_is_ok_none(),
    test_query_bool_truthy(),
    test_path_int_ok(),
    test_path_int_invalid(),
    test_bearer_present(),
    test_bearer_missing(),
  ]
}
