...
fn swagger_ui_html(openapi_url :: Str, title :: Str) -> Str {
  let head := str.concat("<!doctype html><html><head><meta charset=\"utf-8\"><title>", str.concat(title, str.concat("</title><link rel=\"stylesheet\" href=\"", str.concat(swagger_css_url(), str.concat("\"></head><body><div id=\"swagger-ui\"></div><script src=\"", str.concat(swagger_js_url(), "\"></script>"))))))
  let init := str.concat("<script>window.onload=function(){window.ui=SwaggerUIBundle({url:\"", str.concat(openapi_url, "\",dom_id:\"#swagger-ui\",deepLinking:true,layout:\"BaseLayout\"});};</script></body></html>"))
  str.concat(head, init)
}

# ---- ReDoc -------------------------------------------------------
fn redoc_html(openapi_url :: Str, title :: Str) -> Str {
  let head := str.concat("<!doctype html><html><head><meta charset=\"utf-8\"><title>", str.concat(title, str.concat("</title><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><link href=\"https://fonts.googleapis.com/css?family=Roboto:300,400,700|Montserrat:300,400,700\" rel=\"stylesheet\"></head><body><redoc spec-url=\"", str.concat(openapi_url, str.concat("\"></redoc><script src=\"", str.concat(redoc_js_url(), "\"></script></body></html>"))))))
  head
}
...
fn mount(r :: router.Router, openapi_url :: Str, title :: Str) -> router.Router {
  let r1 := router.route(r, "GET", "/docs", fn (_c :: ctx.Ctx) -> resp.Response {
    resp.html(swagger_ui_html(openapi_url, title))
  })
  router.route(r1, "GET", "/redoc", fn (_c :: ctx.Ctx) -> resp.Response {
