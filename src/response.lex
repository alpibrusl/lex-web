...
fn created(location :: Str) -> Response {
  {
    body:    "",
    status:  201,
    headers: map.from_list([("location", location)]),
  }
}
...
fn problem(status :: Int, instance :: Str, errs :: e.Errors) -> Response {
    ...
    errs)
  {
    body:    prob.to_str(p),
    status:  status,
    headers: map.from_list([("content-type", prob.content_type())]),
  }
}
...
fn with_header(resp :: Response, key :: Str, val :: Str) -> Response {
  {
    body:    resp.body,
    status:  resp.status,
    headers: map.set(resp.headers, str.to_lower(key), val),
  }
}

fn with_headers(resp :: Response, pairs :: List[(Str, Str)]) -> Response {
  list.fold(pairs, resp,
    fn (r :: Response, kv :: (Str, Str)) -> Response {
      let k := match kv { (a, _) => a }
      let v := match kv { (_, b) => b }
      with_header(r, k, v)
    })
}
...
fn with_ct(status :: Int, body :: Str, ct :: Str) -> Response {
  { body: body, status: status,
    headers: map.from_list([("content-type", ct)]) }
}
