...
fn build_operation(rec :: router.RouteRecord) -> jv.Json {
  ...
  let base := [
    ("operationId", JStr(operation_id(rec))),
    ("parameters",  JList(params)),
    ("responses",   responses),
  ]
  let with_summary :=
    if str.is_empty(rec.meta.summary) { base }
    else { list.concat(base, [("summary", JStr(rec.meta.summary))]) }
  let with_desc :=
    if str.is_empty(rec.meta.description) { with_summary }
    else { list.concat(with_summary,
             [("description", JStr(rec.meta.description))]) }
  let with_tags := match list.len(rec.meta.tags) {
    0 => with_desc,
    _ => list.concat(with_desc, [("tags",
           JList(list.map(rec.meta.tags,
             fn (t :: Str) -> jv.Json { JStr(t) })))]),
  }
  let with_body := match rec.validator {
    None    => with_tags,
    Some(validator) =>
      list.concat(with_tags, [("requestBody", build_request_body(validator))]),
  }
  JObj(with_body)
}
...
fn operation_id(rec :: router.RouteRecord) -> Str {
  let parts := list.filter(str.split(rec.pattern, "/"),
    fn (s :: Str) -> Bool { not str.is_empty(s) })
  let cleaned := list.map(parts,
    fn (s :: Str) -> Str {
...
fn capitalize(s :: Str) -> Str {
  if str.is_empty(s) { s }
    ...
    str.concat(str.to_upper(str.slice(s, 0, 1)),
...
fn build_tags(routes :: List[router.RouteRecord]) -> List[jv.Json] {
  let names := list.fold(routes, [],
    fn (acc :: List[Str], rec :: router.RouteRecord) -> List[Str] {
      list.fold(rec.meta.tags, acc,
        fn (a :: List[Str], t :: Str) -> List[Str] {
          let already := list.fold(a, false,
            fn (found :: Bool, x :: Str) -> Bool { found or (x == t) })
          if already { a } else { list.concat(a, [t]) }
        })
    })
  list.map(names, fn (n :: Str) -> jv.Json { JObj([("name", JStr(n))]) })
}
...
fn responses_with_success(status :: Int) -> jv.Json {
  ...
  JObj([
    (success_key, JObj([("description", JStr(success_desc))])),
    ("400",       JObj([("description", JStr("Bad Request"))])),
    ("422",       JObj([("description", JStr("Validation Error"))])),
    ("500",       JObj([("description", JStr("Internal Server Error"))])),
  ])
