...
fn inject1[T](
  c   :: ctx.Ctx,
  d1  :: (ctx.Ctx) -> Result[T, resp.Response],
  k   :: (ctx.Ctx, T) -> resp.Response
) -> resp.Response {
  match d1(c) {
    Err(r) => r,
    Ok(v)  => k(c, v),
...
fn inject2[A, B](
  c  :: ctx.Ctx,
  d1 :: (ctx.Ctx) -> Result[A, resp.Response],
  d2 :: (ctx.Ctx) -> Result[B, resp.Response],
  k  :: (ctx.Ctx, A, B) -> resp.Response
) -> resp.Response {
  match d1(c) {
    Err(r) => r,
    Ok(a)  =>
...
fn inject3[A, B, C](
  ...
  d1 :: (ctx.Ctx) -> Result[A, resp.Response],
  d2 :: (ctx.Ctx) -> Result[B, resp.Response],
  d3 :: (ctx.Ctx) -> Result[C, resp.Response],
  k  :: (ctx.Ctx, A, B, C) -> resp.Response
) -> resp.Response {
  match d1(c) {
    Err(r) => r,
    Ok(a)  =>
...
fn inject4[A, B, C, D](
  ...
  d2 :: (ctx.Ctx) -> Result[B, resp.Response],
  d3 :: (ctx.Ctx) -> Result[C, resp.Response],
  d4 :: (ctx.Ctx) -> Result[D, resp.Response],
  k  :: (ctx.Ctx, A, B, C, D) -> resp.Response
) -> resp.Response {
  match d1(c) {
    Err(r) => r,
    Ok(a)  =>
...
#   }
fn bind[A, B](
  prev :: Result[A, resp.Response],
  mk   :: (A) -> Result[B, resp.Response]
) -> Result[B, resp.Response] {
  match prev {
    Err(r) => Err(r),
    Ok(a)  => mk(a),
  }
...
fn pure[T](v :: T) -> Result[T, resp.Response] { Ok(v) }
