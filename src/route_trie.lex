...
type HandlerBody = HPure(((ctx.Ctx) -> resp.Response, Option[v.Validator])) | HEff(((ctx.Ctx) -> [io, time, random, sql, fs_read, fs_write, net, concurrent] resp.Response, Option[v.Validator])) | HStream(((ctx.Ctx) -> [io, time, random, sql, fs_read, fs_write, net, concurrent] stream.StreamResponse, Option[v.Validator]))
