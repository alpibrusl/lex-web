...
fn dispatch_rpc(conn :: WsConn, frame :: Str) -> Str {
  match jv.parse(frame) {
    ...
    Ok(j)  => {
      let id     := jv.get_path_or(j, "id", JNull)
      ...
      if str.is_empty(method) { err_response(id, -32600, "Invalid Request") }
      else { run_method(id, method, params, conn) }
    },
  }
}
...
fn run_method(
  ...
  if method == "ping" { ok_response(id, JStr("pong")) }
  else { if method == "echo" {
  ...
  else { err_response(id, -32601, "Method not found") }
  } } }
}
...
fn http_app() -> router.Router {
  router.new()
    |> fn (r :: router.Router) -> router.Router {
         router.route(r, "GET", "/",
           fn (_c :: ctx.Ctx) -> resp.Response { resp.html(console_html()) })
       }
    |> fn (r :: router.Router) -> router.Router { router.use_mw(r, mw.logger()) }
}

fn handle_http(req :: ctx.RawRequest) -> [io, time] resp.Response {
  router.dispatch(http_app(), req)
}

# ---- Entry point --------------------------------------------------

# Two servers: 8080 for the browser console, 9000 for the WS RPC
# endpoint. Lex's effect system propagates [net, io, time] through
# both because the closure compositions need them.
fn main() -> [net, io, time] Nil {
  let _ := io.print("HTTP   :8080  http://localhost:8080/")
  let _ := io.print("WS-RPC :9000  ws://localhost:9000/rpc/<room>")
  let _ := io.print("Try wscat: > {\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"ping\",\"params\":[]}")
  let _ := ws.serve(9000, "jsonrpc-2.0", on_message)
