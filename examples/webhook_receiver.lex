...
fn webhook_validator() -> v.Validator {
  v.make({
    title: "OrderWebhook", description: "Order lifecycle event",
    fields: [
      s.required_str("event",        [StrPattern("^order\\.[a-z_]+$")]),
      s.required_str("order_id",     [StrNonEmpty, StrMaxLen(64)]),
      s.required_int("amount_cents", [IntNonNegative]),
    ],
  })
}
...
type WebhookErr =
    WhBadSecret
  | WhMissingIdempotencyKey
  | WhDuplicate(Str)
  | WhMissingHeader(Str)

# ---- Dep: shared-secret header check ------------------------------
#
# Real apps would HMAC-verify against the raw body using a shared
# secret stored in the platform — std.crypto.hmac_sha256 lands in
# lex-lang's roadmap. For demo purposes we compare a header verbatim.

fn configured_secret() -> Str { "hush" }
...
fn verify_secret(c :: ctx.Ctx) -> Result[Unit, resp.Response] {
  match ctx.header(c, "x-webhook-secret") {
    ...
    Some(s) =>
      if s == configured_secret() { Ok(()) }
      else { Err(err_response(WhBadSecret)) },
  }
}
...
fn require_idempotency_key(c :: ctx.Ctx) -> Result[Str, resp.Response] {
  match params.header_str(c, "x-idempotency-key", None,
                          [StrMinLen(1), StrMaxLen(128)]) {
    Err(r)  => Err(r),
    Ok(key) => Ok(key),
  }
}
  ...
  exc.new()
    |> fn (r :: exc.Registry[WebhookErr]) -> exc.Registry[WebhookErr] {
         exc.add(r, fn (e :: WebhookErr) -> Option[resp.Response] {
           match e {
             WhBadSecret =>
               Some(resp.unauthorized("invalid x-webhook-secret")),
             _ => None,
           }
         })
       }
    |> fn (r :: exc.Registry[WebhookErr]) -> exc.Registry[WebhookErr] {
         exc.add(r, fn (e :: WebhookErr) -> Option[resp.Response] {
           match e {
             WhMissingHeader(h) =>
               Some(resp.bad_request(str.concat("missing header: ", h))),
             _ => None,
           }
         })
       }
    |> fn (r :: exc.Registry[WebhookErr]) -> exc.Registry[WebhookErr] {
         exc.add(r, fn (e :: WebhookErr) -> Option[resp.Response] {
           match e {
             WhDuplicate(key) =>
               # 200 with the original ack, *not* 4xx — at-least-once
               # delivery means the provider sees the same response
               # for a retry as for the first call.
               Some(resp.json(str.concat(
...
fn handle_webhook(c :: ctx.Ctx, seen :: Map[Str, Unit]) -> resp.Response {
  match depends.bind(verify_secret(c),
    fn (_u :: Unit) -> Result[Str, resp.Response] {
      require_idempotency_key(c)
    })
  {
    Err(r)   => r,
    Ok(key)  =>
      match check_not_duplicate(key, seen) {
        Err(r) => r,
        Ok(_)  =>
          match body.require_json_body(c, webhook_validator()) {
            Err(r)        => r,
...
fn accept_and_queue(key :: Str) -> resp.Response {
  let r := { body: str.concat("{\"status\":\"accepted\",\"idempotency_key\":\"",
                              str.concat(key, "\"}")),
             status: status.HTTP_202_ACCEPTED(),
             headers: map.from_list([("content-type", "application/json")]) }
  bg.with_task(r,
    bg.task(str.concat("process-", key),
      fn () -> [io, time] Nil {
        # In real life: enqueue to a worker, write to DB, emit a
        # domain event, fan out a notification, etc.
...
fn app(seen :: Map[Str, Unit]) -> router.Router {
  router.new()
    |> fn (r :: router.Router) -> router.Router {
         router.handler_json(r, "POST", "/webhooks/order",
           webhook_validator(),
           fn (c :: ctx.Ctx) -> resp.Response { handle_webhook(c, seen) })
       }
    |> fn (r :: router.Router) -> router.Router {
         router.attach_meta(r, "POST", "/webhooks/order", {
           tags:        ["webhooks"],
           summary:     "Receive an order-lifecycle webhook",
           description: "Verifies x-webhook-secret, dedups on x-idempotency-key, queues processing, returns 202.",
           status:      status.HTTP_202_ACCEPTED(),
         })
       }
    |> fn (r :: router.Router) -> router.Router {
    ...
    |> fn (r :: router.Router) -> router.Router { router.use_mw(r, mw.body_limit(64_000)) }
...
fn main() -> [net, io, time] Nil {
  let _ := io.print("webhook receiver on :8080")
  let _ := io.print("  POST /webhooks/order  (needs x-webhook-secret + x-idempotency-key)")
  let _ := io.print("  GET  /healthz")
  let seen := map.new()
  net.serve_fn(8080,
    fn (req :: ctx.RawRequest) -> [io, time] resp.Response {
      router.dispatch(app(seen), req)
    })
}
