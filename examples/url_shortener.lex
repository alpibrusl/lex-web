...
# Slug must be a-z 0-9 dash, 1..40 chars. URL must look like a URL.
# StrPattern + StrUrl give us both checks for free in the 422 path.
fn link_validator() -> v.Validator {
  v.make({
    title: "CreateLink", description: "",
    fields: [
      s.required_str("slug", [StrMinLen(1), StrMaxLen(40),
                              StrPattern("^[a-z0-9-]+$")]),
...
fn seed_store() -> Map[Str, Str] {
  map.from_list([
    ("docs", "https://github.com/alpibrusl/lex-lang"),
    ("web",  "https://github.com/alpibrusl/lex-web"),
  ])
}
...
fn err_registry() -> exc.Registry[LinkErr] {
  exc.new()
    |> fn (r :: exc.Registry[LinkErr]) -> exc.Registry[LinkErr] {
         exc.add(r,
           fn (e :: LinkErr) -> Option[resp.Response] {
             match e {
               LinkNotFound(slug) =>
                 Some(resp.json_status(status.HTTP_404_NOT_FOUND(),
                   str.concat("{\"error\":\"not_found\",\"slug\":\"",
                              str.concat(slug, "\"}")))),
               _ => None,
             }
           })
             ...
             match e {
               SlugTaken(slug) =>
                 Some(resp.json_status(status.HTTP_409_CONFLICT(),
                   str.concat("{\"error\":\"slug_taken\",\"slug\":\"",
                              str.concat(slug, "\"}")))),
               _ => None,
             }
           })
...
fn create_link(c :: ctx.Ctx) -> resp.Response {
  match body.require_json_body(c, link_validator()) {
    Err(r)    => r,
    Ok(_json) =>
      # We re-extract the slug & url from path params on validate; a
      # production handler would decode the validated jv.Json record.
      resp.created_json(
        "{\"slug\":\"go\",\"url\":\"https://example.com\"}",
        "/go"),
  }
}

fn redirect_slug(c :: ctx.Ctx, store :: Map[Str, Str]) -> resp.Response {
  match params.path_str(c, "slug",
                       [StrMinLen(1), StrPattern("^[a-z0-9-]+$")]) {
    Err(r)    => r,
    Ok(slug)  =>
      match lookup(slug, store) {
        Err(e)  => exc.handle(err_registry(), e),
        Ok(url) =>
          # Background-task pattern: queue the click-count
          # increment so the redirect goes out immediately.
          # The dispatcher (or the user's main loop) runs the
          # tasks after the response is sent.
          bg.with_task(resp.redirect(url),
            bg.task("incr-click-count",
              fn () -> [io, time] Nil {
...
fn stats(c :: ctx.Ctx, store :: Map[Str, Str]) -> resp.Response {
  match params.path_str(c, "slug", [StrMinLen(1)]) {
    ...
    Ok(slug)  =>
      match lookup(slug, store) {
        Err(e) => exc.handle(err_registry(), e),
        Ok(u)  => resp.json(str.concat(
          "{\"slug\":\"", str.concat(slug,
          str.concat("\",\"url\":\"", str.concat(u,
          "\",\"clicks\":0}"))))),
      },
  }
}
...
fn api(store :: Map[Str, Str]) -> sub.SubRouter {
  sub.new("/api", ["links"])
    ...
    |> fn (r :: sub.SubRouter) -> sub.SubRouter {
         sub.route(r, "GET", "/stats/:slug",
           fn (c :: ctx.Ctx) -> resp.Response { stats(c, store) })
       }
    |> fn (r :: sub.SubRouter) -> sub.SubRouter {
...
fn openapi_handler(store :: Map[Str, Str]) -> (ctx.Ctx) -> resp.Response {
  fn (_c :: ctx.Ctx) -> resp.Response {
    resp.json(openapi.export_openapi_str(app(store),
      openapi.make_info_full("URL Shortener", "0.1.0",
        "Tiny short-link service built on lex-web v0.2")))
  }
}

fn app(store :: Map[Str, Str]) -> router.Router {
  router.new()
    |> fn (r :: router.Router) -> router.Router { sub.mount(r, api(store)) }
    |> fn (r :: router.Router) -> router.Router {
         router.route(r, "GET", "/:slug",
           fn (c :: ctx.Ctx) -> resp.Response { redirect_slug(c, store) })
       }
    |> fn (r :: router.Router) -> router.Router {
...
fn ls() -> lifespan.Lifespan {
  lifespan.on_startup(lifespan.new(),
    fn () -> [io, time] Nil {
      let _ := io.print("url-shortener on :8080")
      let _ := io.print("  POST /api/links {slug, url}")
      let _ := io.print("  GET  /:slug    (302 redirect)")
      let _ := io.print("  GET  /docs     (Swagger UI)")
      ()
    })
}
