...
# ---- Domain errors ------------------------------------------------
type AppError = AppNotFound(Str) | AppConflict(Str)
...
fn pagination(c :: ctx.Ctx) -> Result[(Int, Int), resp.Response] {
  match params.query_int(c, "page", Some(1), [IntPositive]) {
    Err(r) => Err(r),
    Ok(page) => match params.query_int(c, "size", Some(20), [IntInRange(1, 100)]) {
      Err(r) => Err(r),
...
fn err_registry() -> exceptions.Registry[AppError] {
  (exceptions.new() |> fn (r :: exceptions.Registry[AppError]) -> exceptions.Registry[AppError] {
    exceptions.add(r, fn (e :: AppError) -> Option[resp.Response] {
      match e {
        AppNotFound(what) => Some(resp.json_status(404, str.concat("{\"error\":\"not_found\",\"what\":\"", str.concat(what, "\"}")))),
        _ => None,
      }
    })
    ...
    exceptions.add(r, fn (e :: AppError) -> Option[resp.Response] {
      match e {
        AppConflict(what) => Some(resp.json_status(409, str.concat("{\"error\":\"conflict\",\"what\":\"", str.concat(what, "\"}")))),
        _ => None,
      }
    })
  }
...
fn list_items_h(_c :: ctx.Ctx, _user :: Str, page_size :: (Int, Int)) -> resp.Response {
  ...
  resp.json(str.concat("{\"page\":", str.concat(int.to_str(page), str.concat(",\"size\":", str.concat(int.to_str(size), ",\"items\":[]}")))))
}
...
fn create_item_h(_c :: ctx.Ctx, _user :: Str) -> background.Reply {
  let r := resp.created_json("{\"id\":\"itm_001\",\"name\":\"widget\",\"qty\":3}", "/items/itm_001")
  background.with_task(r, background.task("audit-log", fn () -> [io, time] Nil {
    io.print("audit: created itm_001")
  }))
}
...
fn create_item(c :: ctx.Ctx) -> resp.Response {
  match current_user(c) {
    Err(r) => r,
    Ok(user) => match body.require_json_body(c, tf.item_validator()) {
      Err(rsp) => rsp,
      Ok(_item) => create_item_h(c, user).response,
    },
  }
}
...
fn openapi_handler(_c :: ctx.Ctx) -> resp.Response {
  resp.json(openapi.export_openapi_str(app(), openapi.make_info_full("Items API", "0.2.0", "FastAPI-style demo built on lex-web v0.2.")))
}

fn app() -> router.Router {
  (((((((router.new() |> fn (r :: router.Router) -> router.Router {
    sub.mount(r, items_router())
...
fn ls() -> lifespan.Lifespan {
  (lifespan.new() |> fn (l :: lifespan.Lifespan) -> lifespan.Lifespan {
    lifespan.on_startup(l, fn () -> [io, time] Nil {
      io.print("starting items API on :8080")
    })
...
fn main() -> [net, io, time, crypto, random, sql, fs_read, fs_write, concurrent] Unit {
  let __lex_discard_1 := lifespan.run_startup(ls())
