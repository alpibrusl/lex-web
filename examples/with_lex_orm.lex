...
fn item_schema() -> s.ModelSchema {
  { title: "items", description: "", fields: [s.required_int("id", []), s.required_str("name", [StrNonEmpty, StrMaxLen(64)]), s.required_int("qty", [IntPositive])] }
}

# Body validator — only the writable fields (id is server-assigned).
fn item_write_validator() -> v.Validator {
  v.make({ title: "ItemWrite", description: "", fields: [s.required_str("name", [StrNonEmpty, StrMaxLen(64)]), s.required_int("qty", [IntPositive])] })
}
...
fn bootstrap(db :: conn.Db) -> [sql] Result[Unit, dbe.DbErr] {
  ...
  let sql_stmt := "CREATE TABLE \"items\" (" + "  \"id\" INTEGER PRIMARY KEY AUTOINCREMENT," + "  \"name\" TEXT NOT NULL," + "  \"qty\" INTEGER NOT NULL" + ")"
  let __lex_discard_1 := pending
  Ok(())
}
...
fn current_user(c :: ctx.Ctx) -> Result[Str, resp.Response] {
  match params.bearer(c) {
    ...
    Ok(tok) => if tok == "demo" {
      ...
      Err(resp.unauthorized("invalid bearer token"))
    },
  }
}
...
fn pagination(c :: ctx.Ctx) -> Result[(Int, Int), resp.Response] {
  match params.query_int(c, "page", Some(1), [IntPositive]) {
    Err(r) => Err(r),
    Ok(p) => match params.query_int(c, "size", Some(20), [IntInRange(1, 100)]) {
      Err(r) => Err(r),
      Ok(n) => Ok((p, n)),
    },
  }
...
fn list_items_h(_c :: ctx.Ctx, _user :: Str, page_size :: (Int, Int), db :: conn.Db) -> [sql] resp.Response {
...
fn get_item_h(_c :: ctx.Ctx, id :: Int, db :: conn.Db) -> [sql] resp.Response {
  let plan := q.where_clause(q.select(item_repo()), pr.eq("id", PInt(id)))
...
fn create_item_h(_c :: ctx.Ctx, _user :: Str, body :: jv.Json, db :: conn.Db) -> [sql] resp.Response {
  let plan := q.insert(item_repo(), body)
  match q.run_insert(plan, db) {
    Err(_) => resp.internal_error(),
    Ok(item) => resp.created_json(item_to_json(item), "/items/" + int.to_str(item.id)),
  }
}

# ---- JSON serialisation (manual until lex-schema ships output validators) ----
fn item_to_json(it :: Item) -> Str {
  "{\"id\":" + int.to_str(it.id) + ",\"name\":\"" + it.name + "\"" + ",\"qty\":" + int.to_str(it.qty) + "}"
}

fn items_to_json(items :: List[Item], page :: Int, size :: Int) -> Str {
  let arr := str.join(list.map(items, item_to_json), ",")
  "{\"page\":" + int.to_str(page) + ",\"size\":" + int.to_str(size) + ",\"items\":[" + arr + "]}"
}
...
fn items_router(db :: conn.Db) -> sub.SubRouter {
  ((((((sub.new("/items", ["items"]) |> fn (r :: sub.SubRouter) -> sub.SubRouter {
    sub.route(r, "GET", "/", fn (c :: ctx.Ctx) -> [sql] resp.Response {
      match current_user(c) {
        Err(r) => r,
        Ok(u) => match pagination(c) {
          Err(r) => r,
    ...
    sub.handler_json(r, "POST", "/", item_write_validator(), fn (c :: ctx.Ctx) -> [sql] resp.Response {
      match current_user(c) {
        Err(r) => r,
        Ok(u) => match body.require_json_body(c, item_write_validator()) {
          Err(r) => r,
    ...
    sub.route(r, "GET", "/:id", fn (c :: ctx.Ctx) -> [sql] resp.Response {
...
fn app(db :: conn.Db) -> router.Router {
  (((((router.new() |> fn (r :: router.Router) -> router.Router {
    sub.mount(r, items_router(db))
  }) |> fn (r :: router.Router) -> router.Router {
    router.route(r, "GET", "/openapi.json", fn (_c :: ctx.Ctx) -> resp.Response {
      resp.json(openapi.export_openapi_str(app(db), openapi.make_info_full("Items API", "0.2.0", "lex-web + lex-orm + lex-schema integration demo")))
    })
  }) |> fn (r :: router.Router) -> router.Router {
    ...
    router.use_mw(r, mw.body_limit(1000000))
  }) |> fn (r :: router.Router) -> router.Router {
...
fn main() -> [net, io, time, sql, fs_write] Nil {
  match open_db() {
    ...
    Ok(db) => {
      let __lex_discard_2 := bootstrap(db)
      let __lex_discard_3 := io.print("items API on :8080 (SQLite in-memory)")
      let __lex_discard_4 := io.print("Swagger UI: http://localhost:8080/docs")
      net.serve_fn(8080, fn (req :: ctx.RawRequest) -> [io, time, sql] resp.Response {
